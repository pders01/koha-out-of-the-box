...
Koha bietet die Möglichkeit, für jedes Abonnement Umlauflisten anzulegen. Hierfür werden den Abonnements Benutzer:innen zugeordnet, die Interesse an der Zeitschrift bekundet haben. Die Zeitschrift wird nun nach Erhalt und Einbuchen der Zeitschrift in Koha an die erste Person auf der Umlaufliste gegeben. Die Zeitschrift wandert so von einer Person zur anderen und wird anschließend von der letzten Person wieder an die Bibliothek zurückgegeben.
Um die Funktion nutzen zu können, muss in den Globalen Systemparametern bei Zeitschriften „RoutingSerials“ aktiviert worden sein. Wird die Funktion nicht benötigt, kann diese hier auch deaktiviert und somit ausgeblendet werden.
